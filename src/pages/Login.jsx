...
const Login = () => {
  ...
  const handleLogin = async (dataForm) => {
    ...
    try {
      ...
      if (response) {
        // ✅ CORRECCIÓN: Pasamos los 4 parámetros (token, rol, nombre, APELLIDO)
        auth.setToken(
          response.token, 
          response.rol || rolVisual, 
          response.nombre,
          response.apellido // <-- Agregamos este parámetro que faltaba
        );
  ...
  const handleInvitado = () => {
    // Para invitado pasamos un apellido genérico también
    auth.setToken("TOKEN_INVITADO", "invitado", "Invitado", "ESFOT"); 
    navigate("/dashboard"); 
  };
      ...
      <div className="relative z-10 w-full max-w-md bg-white rounded-3xl shadow-xl p-8 mx-4 my-8 mt-24 border border-gray-200 animate-fadeIn">
        ...
        <form className="space-y-4" onSubmit={handleSubmit(handleLogin)}>
          ...
          <div className="pt-4 space-y-3">
            ...
            <button type="button" onClick={handleInvitado} className="py-3 px-8 bg-[#F5BD45] text-[#17243D] font-extrabold rounded-full hover:shadow-lg hover:bg-yellow-500 w-full text-center transition-all duration-300 active:scale-95">
