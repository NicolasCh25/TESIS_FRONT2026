...
const ChatMessage = ({ message }) => {
  ...
  // ✅ Validación de seguridad para mostrar proyectos: 
  // Debe ser bot, tener el arreglo y que al menos el primer proyecto tenga un PDF válido
  const tieneProyectosValidos = isBot && 
                                message.proyectos?.length > 0 && 
                                message.proyectos[0]?.archivoPDF;
    ...
    <div className={`flex ${isBot ? "justify-start" : "justify-end"} mb-6 animate-fadeIn px-2`}>
      <div className={`flex gap-3 max-w-[88%] ${isBot ? "flex-row" : "flex-row-reverse"}`}>
        ...
        <div className={`p-4 rounded-2xl shadow-md border ${
          ...
          {/* ✅ Tarjetas interactivas - SOLO se muestran si hay proyectos con PDF */}
          {tieneProyectosValidos && (
            <div className="mt-4 space-y-3 border-t border-gray-100 pt-3">
              {message.proyectos.map((pro) => {
                // Si por alguna razón un proyecto de la lista no tiene PDF, no lo renderizamos
                  ...
                  <div key={pro._id} className="bg-gray-50 border-l-4 border-[#F5BD45] p-3 rounded-r-xl shadow-sm">
                    ...
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[9px] text-gray-500 mt-2 font-bold">
                      ...
                      <span className="flex items-center gap-1">
                        <MdComputer size={12} className="text-[#17243D]"/> 
